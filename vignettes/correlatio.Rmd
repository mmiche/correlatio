...
A correlation between two continuous variables can also be visualized geometrically. That is, the Pearson correlation coefficient **is** an angle in the unit circle, nicely shown in this [URL](https://tex.stackexchange.com/questions/233132/diagramming-unit-circle-and-sine-wave-with-tikz). The cosine wave ranges from minus one to plus one, which is the reason why the Pearson correlation coefficient always is within this range.
...
The predictor's length is the square root of column x1, which is 2.796824 (= one unit). The perpendicular (vertical dashed line) crosses the predictor vector at 1.215665. Before computing anything, we can see that the crossing occurs somewhere between one third and one half of the (one unit) length of the predictor vector. Dividing 1.215665 by 2.796824 results in 0.4346591, which is the regression weight estimate.

When scaling both the predictor and the outcome (mean = 0, standard deviation = 1), the b and the beta regression weight are identical, which is this example is 0.5038131:
